...
const CIRCLE_N = 180;
const circlePoints = new THREE.EllipseCurve().getPoints(CIRCLE_N).map(
  p => new THREE.Vector3(p.x, p.y, 0));

const ellipseShapes = {
  sun: new THREE.Line(),
  venus: new THREE.Line(),
  earth: new THREE.Line(),
  mars: new THREE.Line(),
...
function setEllipseShapes(day) {
  ...
  let matrix = ellipseShapes.matrix;
  for (let p of ["venus", "earth", "mars"]) {
    ...
    yAxis = xAxis.map(v => b*v);
    matrix.set(xAxis[0], yAxis[0], zAxis[0], -e*xAxis[0],
               xAxis[1], yAxis[1], zAxis[1], -e*xAxis[1],
               xAxis[2], yAxis[2], zAxis[2], -e*xAxis[2],
               0, 0, 0, 1);
    let pts = circlePoints.map(p => p.applyMatrix4(matrix));
    let geom = ellipseShapes[p].geometry;
    geom.setFromPoints(pts);
    geom.getAttribute("position").needsUpdate = true;
    if (p == "earth") {
      pts = pts.map(p => p.set(-p.x, -p.y, p.z));
...
solidLine.resolution.set(WIDTH, HEIGHT);  // crucial!!
dashedLine.resolution.set(WIDTH, HEIGHT);  // crucial!!
...
window.addEventListener("resize", () => {
  ...
  solidLine.resolution.set(WIDTH, HEIGHT);
  dashedLine.resolution.set(WIDTH, HEIGHT);
  solidLine.resolution.needsUpdate = true;
  dashedLine.resolution.needsUpdate = true;
  render();
}, false);
